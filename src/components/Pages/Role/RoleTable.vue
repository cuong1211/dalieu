            ...
            <tbody class="text-gray-600 fw-bold">
                <template v-if="!loading && roles.length">
                    <tr v-for="(role, index) in roles" :key="role.id">
                        ...
                        <td class="text-left text-gray-800 mb-1">{{ role.name }}</td>
                        <td class="text-left text-gray-800 mb-1">
                            <div v-html="role.description"></div>
                        </td>
                        <td class="text-center text-gray-800 mb-1">{{ role.created_at }}</td>
                        <td class="text-center text-gray-800 mb-1">{{ role.updated_at }}</td>
                        <td class="text-end">
                            <ActionButton :item="role" @edit="$emit('edit', role)"
